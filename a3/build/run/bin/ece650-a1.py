...
def parseLine(line_input):
    ...
    if len(line_ele) == 1:
        if cmd == "gg":
            ...
            sys.stdout.write(output_E() + "\n")
            sys.stdout.flush()
            #print(output_E(), file=sys.stdout)
            ...
            if not existance:
                ...
                for index in range(len(E)):
                    for V_idx in original_coor_Vindex:
                        if E != None and original_coor_Eindex != None:
                            try:
...
# def format_graph_output(V, E):
#     # Number of vertices
#     print(f"V {len(V)}")

#     # Formatting the edges
#     formatted_edges = ",".join(f"<{edge[0]},{edge[1]}>" for edge in E)
#     print(f"E {{{formatted_edges}}}")

# # Call the function with the given dictionaries
# format_graph_output(V, E)
...
def output_E():
    ...
    try:
        if E != None:
            formatted_edges = 'E {' + ','.join(f"<{x},{y}>" for x, y in E) + '}'
        else:
            formatted_edges = "E {" + "}"
    except:
        formatted_edges = "E {" + "}"
...
def output_V():
    ...
    #print("V ={\n" + formatted_V +"\n}")
    #print("V " + str(len(V)), file=sys.stdout)
